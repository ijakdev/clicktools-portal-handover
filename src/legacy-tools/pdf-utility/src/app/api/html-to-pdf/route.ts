...
export async function POST(req: NextRequest) {
    try {
        ...
        // 2. Navigate to URL
        await page.goto(url, {
            waitUntil: 'domcontentloaded', // Wait until HTML is loaded, ignoring hanging requests
            timeout: 60000
        });
        ...
        await page.addStyleTag({
            content: `
                ...
                /* Hide fixed headers/footers that might cover content */
                header.fixed, nav.fixed, .fixed-header { position: absolute !important; }
            `
        });

        // 5. Generate Vector PDF
        // Using scale 0.6 to fit ~1280px desktop view into A4 width
