...
export async function processSmartScan(file: File, onProgress?: (progress: number, status: string) => void): Promise<SmartScanResult> {
    ...
    if (onProgress) onProgress(5, 'OCR 엔진 초기화 중...');
    ...
    try {
        ...
        const worker = await createWorker('eng', 1, {
            ...
            logger: m => {
                if (m.status === 'recognizing text' && onProgress) {
                    onProgress(Math.round(m.progress * 100), `텍스트 분석 중... ${Math.round(m.progress * 100)}%`);
                } else if (onProgress && m.status) {
                    onProgress(0, `상태: ${m.status}`);
                }
            },
        });
        ...
        if (onProgress) onProgress(10, '이미지 분석 실행...');
        ...
        if (onProgress) onProgress(80, 'PDF 생성 중...');

        // In v7, data.pdf is number[] | null. 
        // We do NOT call getPDF().
        if (!data.pdf) {
            throw new Error("PDF 생성에 실패했습니다. (PDF 데이터가 없습니다)");
        }
