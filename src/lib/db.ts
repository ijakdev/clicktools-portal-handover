...
async function seedDb(db: Database) {
    // 1. 초기 관리자 계정 생성 (아이디: admin / 비번: clicktools2026!)
    ...
    // 2. 기존 하드코딩된 블로그 데이터 마이그레이션
    const postCount = await db.get('SELECT COUNT(*) as count FROM blog_posts');
...
5. 와이파이 간편 연결`
        ...
        for (const post of blogPosts) {
            await db.run(
                'INSERT INTO blog_posts (slug, title, excerpt, category, thumbnail, content) VALUES (?, ?, ?, ?, ?, ?)',
                [post.slug, post.title, post.excerpt, post.category, post.thumbnail, post.content]
            );
        }
        console.log('✅ Blog data migrated to database.');
    }

    // 3. URL Shortener 상시 누적 카운터 초기화
    const creationStat = await db.get('SELECT value FROM global_stats WHERE name = ?', ['url_shortener_total_creations']);
    if (!creationStat) {
        const urlCount = await db.get('SELECT COUNT(*) as count FROM urls');
        await db.run('INSERT INTO global_stats (name, value) VALUES (?, ?)', ['url_shortener_total_creations', urlCount.count || 0]);
...
export async function getDb() {
    ...
    // Schema Check and Column Migration
    const tableInfo = await dbInstance.all("PRAGMA table_info(blog_posts)");
    const columns = tableInfo.map(c => c.name);

    if (!columns.includes('image1')) {
        await dbInstance.exec(`
            ALTER TABLE blog_posts ADD COLUMN image1 TEXT;
            ALTER TABLE blog_posts ADD COLUMN image1_alt TEXT;
            ALTER TABLE blog_posts ADD COLUMN image1_caption TEXT;
            ALTER TABLE blog_posts ADD COLUMN image2 TEXT;
            ALTER TABLE blog_posts ADD COLUMN image2_alt TEXT;
            ALTER TABLE blog_posts ADD COLUMN image2_caption TEXT;
            ALTER TABLE blog_posts ADD COLUMN image3 TEXT;
            ALTER TABLE blog_posts ADD COLUMN image3_alt TEXT;
            ALTER TABLE blog_posts ADD COLUMN image3_caption TEXT;
        `);
    }
